...
function Networkmetrics(props) {
  const [data, setData] = useState(null);

  useEffect(() => {
    axios.get(`http://localhost:3001/getParams/networkMetrics?time=${props.time}&ip=${props.ip}`)
      .then(response => {
  ...
  return (
    ...
    {data ? <MetricsDisplay metrics={data} heading='Network Metrics'/> : <p>Loading...</p>}
    </div>
  );
}
